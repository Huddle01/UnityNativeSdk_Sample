...
namespace Huddle01.Core
{
    public class LocalPeer : EventEmitter
    {
        ...
        private void HelloReceived(Hello hello)
        {
            ...
            _ = Room.Instance.Emit("connected-to-server");
        }
        ...
        void ReceiveDataReceived(ReceiveData receiveData)
        {
            ...
            Emit("receive-data", receiveData.From, receiveData.Label, receiveData.Payload);
        }
        ...
        void PeerMetadataUpdatedReceived(PeerMetadataUpdated peerMetadataUpdated)
        {
            ...
            try
            {
                ...
                _ = Room.Instance.Emit("peer-metadata-updated", peerId);
            }
            catch (Exception ex)
            {
        ...
        private void RoomClosedProducersReceived(RoomClosedProducers roomClosedProducers)
        {
            ...
            try
            {
                ...
                Room.Instance.Emit("room-notification", new Dictionary<string, object>
                {
        ...
        {
            _metadata = metadata;
            if (!_permissions.CheckPermission(PermissionType.canUpdateMetadata, null)) return;

            if (!_joined) Debug.LogError("Cannot Update Metadata, You have not joined the room yet");

            if(string.IsNullOrEmpty(_peerId)) Debug.LogError("Cannot Update Metadata, peerid null");

            _webSocketService.SendRequest(_webSocketService.RequestPublisher.UpdatePeerMetadataRequest(_peerId, metadata));
        ...
        public async Task StopProducing(string label)
        {
            ...
            if (producer != null)
            {
                ...
                if (closedStream)
                {
                    _ = Emit("stream-closed", label, "1200", "STREAM_CLOSED", "Stopped Streaming");
                }
            }
        }
